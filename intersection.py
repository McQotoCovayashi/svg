...
def Intersection(Point1, Vector1, Point2, Vector2, plot = False, label = 'intersection Point', color = 'c', marker = 'x', s = 50):
    if la.norm(Vector1)==0 or la.norm(Vector2)==0 :
        raise Exception("Vectro1 or Vector2 are ZERO Vector. An intersection Point is NOT exist.")
    if Vector1[1] == 0 or Vector2[1] == 0:
        pass    
    elif Vector1[0]/Vector1[1]==Vector2[0]/Vector2[1]:
        raise Exception("Vectro1 and Vector2 are PARALLEL. An intersection Point is NOT exist.")
